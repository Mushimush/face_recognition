...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # Get the embeddings of the face using the FaceNet model
        ...
        # Make a prediction using the normalized embeddings
        pred = new_model.predict(embeddings)
        # Get the label with the highest probability
        label = np.argmax(pred)

        # Draw a rectangle around the face and display the label
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, labels[label], (x, y-10),
